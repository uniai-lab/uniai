...
describe('AliYun QWen Tests', () => {
    ...
    test('Test chat AliYun QWen with vision plus', done => {
        uni.chat(input2, { stream: true, provider: ChatModelProvider.AliYun, model: AliChatModel.QWEN_VL_PLUS }).then(
            res => {
                expect(res).toBeInstanceOf(Readable)
                const stream = res as Readable
                let data = ''
    ...
    test('Test chat AliYun QWen with vision max', done => {
        uni.chat(input2, { provider: ChatModelProvider.AliYun, model: AliChatModel.QWEN_VL_MAX })
            .then(console.log)
            .catch(console.error)
            .finally(done)
    }, 60000)
