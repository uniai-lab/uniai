...
import { ChatModelProvider, ChatRoleEnum, EmbedModelProvider } from '../interface/Enum'
...
describe('Other Tests', () => {
    beforeAll(() => {
        uni = new UniAI({ Other: { api: OTHER_API, key: OTHER_KEY } })
    })

    test.each(otherChatModels)(
        'Test chat other model %s',
        async model => {
            const res = await uni.chat(input, { stream: false, provider: ChatModelProvider.Other, model })
            console.log(res)
        },
        60000
    )
    ...
    test('Test chat other model stream', done => {
        ...
        uni.chat(input, {
            ...
            stream.on('data', chunk => {
                const res = JSON.parse(chunk.toString()) as ChatResponse
                process.stdout.write(res.content)
                data.id = res.id
                data.model = res.model
                data.content += res.content
                data.promptTokens += res.promptTokens
                data.completionTokens += 1
                data.totalTokens += res.totalTokens
    ...
    // --------------------------------------embedding tests--------------------------------------
    test('Test Other bge-m3:latest', done => {
        uni.embedding([input, input], { provider: EmbedModelProvider.Other, model: 'bge-m3:latest' })
            .then(res => {
                console.log(res)
                return res
            })
    ...
    test('Test Other embeddinggemma:latest', done => {
        uni.embedding([input, input], { provider: EmbedModelProvider.Other, model: 'embeddinggemma:latest' })
            .then(res => {
                console.log(res)
                return res
            })
    ...
    test('Test Other qwen3-embedding:4b', done => {
        uni.embedding([input, input], { provider: EmbedModelProvider.Other, model: 'qwen3-embedding:4b' })
            .then(res => {
                console.log(res)
                return res
            })
